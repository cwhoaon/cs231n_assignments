...
def general_forward(x, w, b, normalization=None, gamma=None, beta=None, norm_param=None, dr_param=None, last=False):
    fc_cache, bn_cache, ln_cache, relu_cache, dr_cache = (None for i in range(5))
    ...
    if not last:
        ...
        if dr_param != None:
            out, dr_cache = dropout_forward(out, dr_param)

    cache = fc_cache, bn_cache, ln_cache, relu_cache, dr_cache

    return out, cache

def general_backward(dout, cache):
    fc_cache, bn_cache, ln_cache, relu_cache, dr_cache = cache
    dx, dw, db, dgamma, dbeta = (None for i in range(5))
